...
def preprocess(df):
    df = df.copy()
    df = df.fillna(df.median(numeric_only=True))  # Simple imputation for numeric feature
    return df
...
X_final = test_data.drop('Outcome', axis=1, errors='ignore').fillna(X_train_fe.median())
