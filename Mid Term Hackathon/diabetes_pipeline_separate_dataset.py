...
#========================================================================
train = pd.read_csv('DiabetesTrain.csv')
test  = pd.read_csv('DiabetesTest.csv')
...
def preprocess(df):
    df = df.copy()
    # No categorical features to encode, no specific columns to drop for Diabetes
...
# Align columns — test may be missing some dummy columns (Not applicable here, but good practice)
train_data, test_data = train_data.align(test_data, join='left', axis=1, fill_value=0)

X_fe = train_data.drop('Outcome', axis=1).fillna(train_data.median(numeric_only=True))
y_fe = train_data['Outcome']
print(X_fe.dtypes)
...
# Note: 'Outcome' column won't exist in test data normally, so drop if it does.
X_final = test_data.drop('Outcome', axis=1, errors='ignore').fillna(X_train_fe.median(numeric_only=True))
...
output = pd.DataFrame({'predicted_outcome': final_pred})
output.to_csv('final_diabetes_predictions.csv', index=False)
print('Predictions for the separate test dataset saved to final_diabetes_predictions.csv')
